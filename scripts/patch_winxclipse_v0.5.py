...
if len(sys.argv) not in (4, 5):
    print("Uso: patch_winxclipse_v0.5.py base.apk icon.png output-unsigned.apk [leegao_wrapper.tar.xz]")
    sys.exit(1)

base_apk = Path(sys.argv[1])
icon_path = Path(sys.argv[2])
out_apk = Path(sys.argv[3])
leegao_wrapper = Path(sys.argv[4]) if len(sys.argv) == 5 else None

replacements = {
    "Winlator Cmod": "WinXclipse   ",
    "Winlator CMOD": "WinXclipse   ",
    "Version Cmod-v13.1": "Version WinX-v0.5 ",
    "Version 7.1.4x-cmod": "Version WinX-v0.5  ",
    "Cmod-v13.1": "WinX-v0.5 ",
}
...
with zipfile.ZipFile(base_apk, "r") as zin, zipfile.ZipFile(out_apk, "w", compression=zipfile.ZIP_DEFLATED) as zout:
    ...
    wrote_leegao = False
    if leegao_wrapper is not None:
        if not leegao_wrapper.is_file():
            raise FileNotFoundError(f"Wrapper Leegao não encontrado: {leegao_wrapper}")

        zout.writestr(
            "assets/graphics_driver/libvulkan_wrapper_leegao_etc2.tar.xz",
            leegao_wrapper.read_bytes()
        )
        wrote_leegao = True

print("substituições de texto:", changed_text)
print("ícones substituídos:", changed_icons)
print("Leegao ETC2 adicionado:", wrote_leegao)
